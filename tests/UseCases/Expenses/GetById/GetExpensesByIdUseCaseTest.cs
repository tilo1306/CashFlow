...
namespace UseCases.Expenses.GetById;
...
public class GetExpensesByIdUseCaseTest
{
  ...
  private GetIdExpenseUseCase CreateUseCase(User user, Expense? expense = null)
  {
    var repository = new ExpensesReadOnlyRepositoryBuilder().GetById(user, expense).Build();
    var mapper = MapperBuilder.Build();
    var loggedUser = LoggedUserBuilder.Build(user);
    
    return new GetIdExpenseUseCase(repository, mapper, loggedUser);
  }
}
